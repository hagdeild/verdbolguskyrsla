...
verdbolguspar_tbl <- verdbolguspar_tbl |>
  select(-Seðlabankinn) |>
  filter(name %in% c("Söguleg", "Skammtíma"))


# sviðsmyndir
svidsmyndir_tbl <- read_csv("data/greiningar/oliuverd_scenarios.csv") |>
  ...
  mutate(
    duration = 
      case_when(
        ...
        duration == "2 month" ~ "Tímabundin hækkun í 2 mánuði",
        duration == "3 month" ~ "Tímabundin hækkun í 3 mánuði",
        TRUE ~ "Varanleg hækkun"
      )
  ) |> 
