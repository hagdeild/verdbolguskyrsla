...
# 12 mánaða spá
fc_diff_tbl <- fc_ls$verdbolguspa |> 
    filter(!name == "Söguleg") |> 
    arrange(date, name) |> 
    group_by(name) |> 
...
sum_fc_diff <- sum(fc_diff_tbl$diff > 0)

# 1 mánaða spá
fc_1m <- str_extract_all(fc_ls$verdbolguspa_valuebox$spa_1m, "-?\\d+\\.\\d+")[[1]] |> as.numeric()
sum_fc_1m_diff <- sum(fc_1m > 0)
```
...
list(
    icon = case_when(
        sum_fc_diff == 2 ~ "arrow-up",
        sum_fc_diff == 1 ~ "dash",
        TRUE ~ "arrow-down"
        ),
    color = 
        case_when(
        sum_fc_diff == 2 ~ valuebox_litir[2],
        sum_fc_diff == 1 ~ valuebox_litir[3],
...
list(
    icon = case_when(
        sum_fc_1m_diff == 2 ~ "arrow-up",
        sum_fc_1m_diff == 1 ~ "dash",
        TRUE ~ "arrow-down"
        ),
    color = 
        case_when(
        sum_fc_1m_diff == 2 ~ valuebox_litir[2],
        sum_fc_1m_diff == 1 ~ valuebox_litir[3],
        TRUE ~ valuebox_litir[1]
        ),
...
p18 <- fc_ls$verdbolguspa |> 
    ...
    scale_y_continuous(labels = percent) +
    theme_minimal() +
    scale_color_manual(values = vr_litir[c(2, 4, 1)])
