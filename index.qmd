...
vr_litir <- c("#0072b1", "#54b6e9", "#00a073", "#f0e142", "#e59e23", "#d65e18", "#D64550")

valuebox_litir <- c("#00B894", "#FF6B6B", "#FDCB6E") #c("#2E7D6E", "#E57373", "#F9A825")
...
p2 <- data_ls$pbi_inflation_verdbolga |> 
    ...
    geom_point(aes(date, mom, colour = "Spá VR fyrir viðkomandi mánuði á þessu ári", text = paste0("date: ", date, "\nspá: ", merking_mom)),
               shape = 17, size = 4) +
    scale_colour_manual(values = setNames(vr_litir[3], "Spá VR fyrir viðkomandi mánuði á þessu ári")) +
    labs(
        x = NULL,
        y = NULL,
        title = "Verðbólga milli mánaða"
...
for (i in seq_along(p2_plotly$x$data)) {
  ...
  if (!is.null(tr$mode) && grepl("markers", tr$mode)) {
    ...
    p2_plotly$x$data[[i]]$textfont <- list(color = vr_litir[3], size = text_size)
...
waterfall_plot(
  data_ls$pbi_inflation_waterfall |> drop_na(),
  ahrif,
  title = "Framlag undirliða seinustu 12 mánuði",
  flip = TRUE,
  total_value = verdbolga_total_value,
  label_digits = 2
) +
    scale_y_continuous(labels = percent)
...
waterfall_plot(
  data_ls$phi_inflation_undirflokkar_1m |> set_names("date", "undirflokkur", "ahrif_1m") |> mutate(ahrif_1m = ahrif_1m/100),
  ahrif_1m,
  title = "Framlag undirliða seinasta mánuð",
  flip = TRUE,
  label_digits = 2,
  total_value = verdbolga_milli_manada
) +
    scale_y_continuous(labels = percent)
